(function() {
  if (
    window.location.href.includes("product-t160") ||
    window.location.href.includes("product-b90") ||
    window.location.href.includes("product-t90")
  ) {
    ...
    var calcResultsProductsPerShelf = document.querySelector(
      "#calc-results-products-per-shelf"
    );
    ...
    function calculate(e) {
      ...
      if (height === width) {
        ...
        if (result1.totalProductCapacity >= result2.totalProductCapacity)
          results = result1;
    ...
    function calculateStorageSpace(height, width, depth) {
      var totalHeight =
        height +
        unitConst[currentUnits].HEAVY_DUTY_WIRE_RACK +
        unitConst[currentUnits].HEIGHT_SPACING;
      ...
      if (window.location.href.includes("product-t90")) {
        results = calculateTotals(
          {
            height: unitConst[currentUnits].T_90_HEIGHT,
            width: unitConst[currentUnits].T_90_WIDTH,
            depth: unitConst[currentUnits].T_90_DEPTH
          },
          shelfSpacing,
          totalWidth,
          totalDepth
        ...
        results = calculateTotals(
          {
            height: unitConst[currentUnits].T_160_HEIGHT,
            width: unitConst[currentUnits].T_160_WIDTH,
            depth: unitConst[currentUnits].T_160_DEPTH
          },
          shelfSpacing,
          totalWidth,
          totalDepth
        ...
        results = calculateTotals(
          {
            height: unitConst[currentUnits].B_90_HEIGHT,
            width: unitConst[currentUnits].B_90_WIDTH,
            depth: unitConst[currentUnits].B_90_DEPTH
          },
          shelfSpacing,
          totalWidth,
          totalDepth
